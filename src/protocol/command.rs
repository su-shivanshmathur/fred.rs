...
pub enum RouterCommand {
  ...
  /// Abort the reader task for a random connection. Returns true if a reader was found and aborted.
  AbortReader {
...
impl fmt::Debug for RouterCommand {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    ...
    match self {
      ...
      RouterCommand::AbortReader { .. } => {
        formatter.field("kind", &"Abort Reader");
      },
    };
